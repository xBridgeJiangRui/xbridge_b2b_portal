...
class b2b_billing_invoice_controller extends CI_Controller {
    ...
    public function invoice_tb_admin()
    {
        ...
        {
            $condition = " WHERE b.`status` = '$status_data' AND a.inv_status = 'Emailed'";
        }
        else if($status_data != '' && $period_code != '' && $supplier_guid == '')
        {
            $condition = " WHERE b.`status` = '$status_data' AND a.inv_status = 'Emailed' AND a.period_code IN ('$period_code') ";
        }
        else if($status_data != '' && $period_code != '' && $supplier_guid != '')
        {
            $condition = " WHERE b.`status` = '$status_data' AND a.inv_status = 'Emailed' AND a.period_code IN ('$period_code') AND a.biller_guid = '$supplier_guid'  ";
        }
        else if($status_data == '' && $period_code != '' && $supplier_guid == '')
        {
            $condition = "WHERE a.period_code IN ('$period_code')";
        }
        else if($status_data == '' && $period_code != '' && $supplier_guid != '')
        {
            $condition = "WHERE a.period_code IN ('$period_code') AND a.biller_guid = '$supplier_guid'";
        }
        else if($status_data == '' && $period_code == '' && $supplier_guid != '')
        {
