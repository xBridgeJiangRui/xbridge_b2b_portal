...
class Supplier_setup_new extends CI_Controller
{
    ...
    public function acc1_table()
    {
        ...
        $get_user_guid = $_SESSION['user_guid'];
        $draw = intval($this->input->post("draw"));
        $start = intval($this->input->post("start"));
        $length = intval($this->input->post("length"));
        $order = $this->input->post("order");
        ...
        if ($variable == 'supplier_guid') //if using highlight supplier mode
        {
            ...
            $sql = "SELECT a.acc_guid, a.user_guid, b.supplier_guid, user_id,  user_name, f.supplier_name , all_sup_assigned ,  f.`name_reg`, IFNULL(g.user_group_name,'') AS user_group_name  FROM  ( SELECT  `acc_guid`, `branch_guid`, `module_group_guid`, `user_group_guid`, a.`user_guid`, a.`supplier_guid`, `user_id`, `user_password`, `user_name`, a.`created_at`, a.`created_by`, a.`updated_at`, a.`updated_by` , `supplier_name`, `reg_no`, `gst_no`, `name_reg` FROM set_user  AS a  LEFT JOIN set_supplier AS b  ON a.supplier_guid = b.supplier_guid WHERE a.acc_guid = '" . $_SESSION['customer_guid'] . "' GROUP BY a.user_guid ORDER BY a.updated_at DESC)  AS a INNER JOIN ( SELECT supplier_guid,user_guid , GROUP_CONCAT(supplier_group_name) AS all_sup_assigned, acc_guid FROM `check_user_supplier_customer_relationship` where acc_guid='" . $_SESSION['customer_guid'] . "' and  supplier_guid = '$guid'  GROUP BY user_guid,  acc_guid ) b ON a.user_guid = b.user_guid INNER JOIN set_supplier f ON b.supplier_guid = f.supplier_guid LEFT JOIN lite_b2b.set_user_group g ON a.user_group_guid = g.user_group_guid ";
        } else {
            if ($variable == 'customer_guid') // if using customer filter mode
            {
                $sql =  "SELECT b.supplier_guid,a.acc_guid, a.user_guid,user_id,  user_name, f.supplier_name , all_sup_assigned ,  f.`name_reg`, IFNULL(g.user_group_name,'') AS user_group_name  FROM  ( SELECT  `acc_guid`, `branch_guid`, `module_group_guid`, `user_group_guid`, a.`user_guid`, a.`supplier_guid`, `user_id`, `user_password`, `user_name`, a.`created_at`, a.`created_by`, a.`updated_at`, a.`updated_by` , `supplier_name`, `reg_no`, `gst_no`, `name_reg` FROM set_user  AS a  LEFT JOIN set_supplier AS b  ON a.supplier_guid = b.supplier_guid WHERE a.acc_guid = '$guid' GROUP BY a.user_guid ORDER BY a.updated_at DESC )  AS a inner JOIN ( SELECT supplier_guid,user_guid , GROUP_CONCAT(supplier_group_name) AS all_sup_assigned FROM `check_user_supplier_customer_relationship` where acc_guid = '$guid'  GROUP BY user_guid,  acc_guid ) b ON a.user_guid = b.user_guid LEFT JOIN set_supplier f ON b.supplier_guid = f.supplier_guid LEFT JOIN lite_b2b.set_user_group g ON a.user_group_guid = g.user_group_guid ";

            } else {

                $sql =  "SELECT b.supplier_guid,a.acc_guid, a.user_guid, user_id,  user_name, f.supplier_name , all_sup_assigned ,  f.`name_reg`, a.user_group_name  FROM  ( SELECT  a.`acc_guid`, a.`branch_guid`, a.`module_group_guid`, a.`user_group_guid`, a.`user_guid`, a.`supplier_guid`, `user_id`, `user_password`, `user_name`, a.`created_at`, a.`created_by`, a.`updated_at`, a.`updated_by` , `supplier_name`, `reg_no`, `gst_no`, `name_reg`, IFNULL(c.user_group_name, '') AS user_group_name FROM set_user  AS a  LEFT JOIN set_supplier AS b  ON a.supplier_guid = b.supplier_guid INNER JOIN lite_b2b.set_user_group c ON a.user_group_guid = c.user_group_guid  GROUP BY a.user_guid,a.acc_guid ORDER BY a.updated_at DESC )  AS a LEFT JOIN ( SELECT supplier_guid,user_guid , GROUP_CONCAT(supplier_group_name) AS all_sup_assigned, acc_guid FROM `check_user_supplier_customer_relationship` GROUP BY user_guid,  acc_guid ) b ON a.user_guid = b.user_guid AND a.acc_guid = b.acc_guid LEFT JOIN set_supplier f ON b.supplier_guid = f.supplier_guid ";
            } //clsoe else
        }
    ...
    public function delete_group()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '') {
            ...
            $get_supplier_group_data = $this->db->query("SELECT * FROM lite_b2b.set_supplier_group WHERE customer_guid = '$customer_guid' and supplier_group_guid = '$supplier_group_guid' and supplier_guid = '$supplier_guid'")->result_array();

            if(count($get_supplier_group_data) > 0)
            {
                foreach($get_supplier_group_data as $row => $value)
                {
                    ...
                    $data = array(
                        ...
                        'supplier_code_guid' => $value['supplier_group_guid'],
                        'supplier_code' => $value['supplier_group_name'],
                        'created_at' => $value['created_at'],
                        'created_by' => $value['created_by'],
                        'deleted_at' => $this->db->query("SELECT now() as today")->row('today'),
                        'deleted_by' => $user_name,
                    );
                    $this->db->insert('lite_b2b.set_supplier_group_log', $data);
                }
            }
    ...
    public function get_supplier_multiple()
    {
        ...
        foreach ($result->result() as $row) {
            if (isset($_REQUEST['customer_guid'])) //if using highlight supplier mode
            {
                ...
                $supplier_code = $this->db->query("SELECT a.*,c.acc_doc_name FROM lite_b2b.set_supplier_group a LEFT JOIN b2b_summary.supcus b ON a.supplier_group_name = b.code AND a.customer_guid = b.customer_guid LEFT JOIN lite_b2b.acc c ON b.customer_guid = c.acc_guid WHERE a.supplier_guid = '$row->supplier_guid' AND a.customer_guid = '$customer_guid'");
                ...
                $supplier_code = $this->db->query("SELECT a.*,c.acc_doc_name FROM lite_b2b.set_supplier_group a LEFT JOIN b2b_summary.supcus b ON a.supplier_group_name = b.code AND a.customer_guid = b.customer_guid LEFT JOIN lite_b2b.acc c ON b.customer_guid = c.acc_guid WHERE a.supplier_guid = '$row->supplier_guid'");
            ...
            if ($i == 0) {
                ...
                foreach ($supplier_code->result() as $row2) {
                    if (in_array($row2->supplier_group_guid, $selected_supplier_code_array)) {
                        $dropdown .= '<option selected required data-default value="' . $row2->supplier_group_guid . '">' . $row2->supplier_group_name . ' - '. $row2->acc_doc_name.'</option>';
                    } else {
                        $dropdown .= '<option required data-default value="' . $row2->supplier_group_guid . '">' . $row2->supplier_group_name . ' - '. $row2->acc_doc_name.'</option>';
                    }
                }
                $dropdown .= '</select></div></div>';
            } else {
