...
class Consignment_report extends CI_Controller
{
    ...
    public function consignment_sales_statement_view()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            $refno = str_replace(" ", "%20",$consignment_sales_statement_refno);
    ...
    public function consignment_sales_statement_by_supcode_list()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            {
                $target_dir = $file_path_name . $path_seperator . $customer_guid . $path_seperator . 'consign_inv' . $path_seperator . $target_dir_supplier_guid . $path_seperator . $company_id . $path_seperator . $ssupcode . '_' . $speriod_code . '.pdf';
            }
    ...
    public function header_save_inv_no_by_supcode()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            if ($check_supplier_inv_no->num_rows() > 0) {
                $data = array(
                    'para1' => 1,
                    'msg' => 'Supplier Invoice No Cannot Duplicate.',
                );
    ...
    public function consignment_generate_e_invoices_by_supcode()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            if(strpos($b2b_inv_no, ',') !== false ) 
            {
                ...
                redirect(site_url('Consignment_report/consignment_sales_statement_by_supcode_list').'?status='.$status.'&loc='.$loc.'&period_code='.$period_code.'&supcode='.$supcode.'&date_trans='.$date_trans.'&company_name='.str_replace(' ','%20',$company_name));
            }

            if(strlen($b2b_inv_no) > '20')
            {
                $this->session->set_flashdata('warning', 'E invoices not generated, Supplier Invoice length exceed limit 20 characters.');
                redirect(site_url('Consignment_report/consignment_sales_statement_by_supcode_list').'?status='.$status.'&loc='.$loc.'&period_code='.$period_code.'&supcode='.$supcode.'&date_trans='.$date_trans.'&company_name='.str_replace(' ','%20',$company_name));
            }
