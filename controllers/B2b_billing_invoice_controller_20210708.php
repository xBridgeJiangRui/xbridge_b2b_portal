...
class b2b_billing_invoice_controller extends CI_Controller {
        ...
        {  
            ...
            if(in_array('IAVA',$this->session->userdata('module_code'))) 
            {
                ...
                $detail_list = $this->db->query("SELECT reg_no,period_code, acc_doc_name , c.supplier_name, a.supplier_guid, customer_guid,
                            ...
                            , IF(doc_type = 'PCN', 'Purchase Credit Note','Others'
                            ...
                            WHERE  hide_supplier_invoice = '0' AND isissued = '1' ".$query_isall." AND a.customer_guid = '$customer_guid' AND doc_type != 'CONSIGN' GROUP BY a.customer_guid,a.supplier_guid,a.doc_type,a.period_code      
                            UNION ALL         
                            ...
                            LEFT JOIN lite_b2b.set_supplier AS c
                            ON a.`supplier_guid` = c.supplier_guid
                            WHERE  hide_supplier_invoice = '0' AND isissued = '1' ".$query_isall." AND a.customer_guid = '$customer_guid' GROUP BY a.customer_guid,a.supplier_guid,a.doc_type,a.period_code ORDER BY doc_type ASC");
            ...
            //                 AND hide_supplier_invoice = '0' AND isissued = '1' ".$query_isall." AND a.customer_guid = '$customer_guid' GROUP BY a.customer_guid,a.supplier_guid,a.doc_type,a.period_code 
            //                 UNION ALL         
            //                 SELECT reg_no,period_code, acc_doc_name , c.supplier_name, a.supplier_guid, customer_guid,
            //                 'Consign outlet' AS doc_count,SUM(doc_count) AS doc_count 
            //                 FROM b2b_invoice.supplier_monthly_doc_count_consignment  AS a
            ...
            else //supp admin clerk and limited supp admin
            {
                $detail_list = $this->db->query("SELECT c.reg_no,a.period_code, acc_doc_name , c.supplier_name, a.supplier_guid, customer_guid,
                            ...
                            LEFT JOIN lite_b2b.set_supplier AS c
                            ON a.`supplier_guid` = c.supplier_guid
                            INNER JOIN b2b_invoice.supplier_monthly_main d
                            ON a.supplier_guid = d.biller_guid AND a.period_code = d.period_code
                            WHERE a.supplier_guid IN (".$supplier_guid.")
                            AND hide_supplier_invoice = '0' AND isissued = '1' AND d.inv_status != 'New' ".$query_isall." AND a.customer_guid = '$customer_guid' AND doc_type != 'CONSIGN' GROUP BY a.customer_guid,a.supplier_guid,a.doc_type,a.period_code
                            UNION ALL         
                            ...
                            ON a.supplier_guid = d.biller_guid AND a.period_code = d.period_code
                            WHERE a.supplier_guid IN (".$supplier_guid.")
                            AND hide_supplier_invoice = '0' AND isissued = '1' AND d.inv_status != 'New' ".$query_isall." AND a.customer_guid = '$customer_guid' GROUP BY a.customer_guid,a.supplier_guid,a.doc_type,a.period_code");
                            // AND hide_supplier_invoice = '0' AND isissued = '1' ".$query_isall." order by 
    ...
    
    {
        ...
        if($period_code <= '2020-02' || $_SESSION['customer_guid'] == '13EE932D98EB11EAB05B000D3AA2838A')
        {
        ...
        {
            if ($doc_type == 'Purchase Order') {
                $data = $this->db->query("SELECT a.refno,b.loc_group as loc_group, a.postdatetime FROM b2b_invoice.supplier_monthly_doc_count_po AS a INNER JOIN b2b_summary.pomain b ON a.refno = b.refno AND b.customer_guid = '$customer_guid' WHERE a.period_code = '$period_code' AND a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid'")->result();
            } else if ($doc_type == 'Goods Received Note') {

                $data = $this->db->query("SELECT a.refno, b.loc_group AS loc_group, a.postdatetime FROM b2b_invoice.supplier_monthly_doc_count_gr AS a INNER JOIN b2b_summary.grmain b ON a.refno = b.refno AND b.customer_guid = '$customer_guid' WHERE a.period_code = '$period_code' AND a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid'")->result();
                
            } else if ($doc_type == 'Promo Tax Inv'){

                $data = $this->db->query("SELECT a.refno, b.loc_group as loc_group, a.postdatetime AS postdatetime FROM b2b_invoice.supplier_monthly_doc_count_promo_taxinv AS a INNER JOIN b2b_summary.promo_taxinv b ON a.refno = b.inv_refno AND b.customer_guid = '$customer_guid' WHERE a.period_code = '$period_code' AND a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid'")->result();


            } else if ($doc_type == 'Credit Note'){

                $data = $this->db->query("SELECT a.refno, b.locgroup AS loc_group, a.postdatetime FROM b2b_invoice.supplier_monthly_doc_count_cn AS a INNER JOIN b2b_summary.cnnotemain b ON a.refno = b.refno AND b.customer_guid = '$customer_guid' WHERE a.period_code = '$period_code' AND a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid'")->result();


            } else if ($doc_type == 'Return Note'){

                $data = $this->db->query("SELECT a.refno, b.locgroup AS loc_group, a.postdatetime FROM b2b_invoice.supplier_monthly_doc_count_dbnote AS a INNER JOIN b2b_summary.dbnotemain b ON a.refno = b.refno AND b.customer_guid = '$customer_guid' WHERE a.period_code = '$period_code' AND a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid'")->result();


            } else if ($doc_type == 'Display Incentive'){

                $data = $this->db->query("SELECT a.refno AS refno, b.loc_group, a.postdatetime AS postdatetime FROM b2b_invoice.supplier_monthly_doc_count_discheme_taxinv a INNER JOIN b2b_summary.discheme_taxinv b ON a.refno = b.inv_refno AND b.customer_guid = '$customer_guid' WHERE a.period_code = '$period_code' AND a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid'")->result();


            }else if ($doc_type == 'Accounting Document'){

                $data = $this->db->query("SELECT b.refno AS refno, doctype AS loc_group, a.postdatetime AS postdatetime FROM b2b_invoice.supplier_monthly_doc_count_acc_doc a INNER JOIN b2b_summary.other_doc b ON a.refno = b.refno AND a.doc_type = b.doctype WHERE a.period_code = '$period_code' AND a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid' AND b.customer_guid = '$customer_guid' GROUP BY a.doc_type,a.refno")->result();
            ...
            {     
                ...
                if($supplier_guid != '')
                {
                    ...
                    {                
                        ...
                        // echo $con_deb_code;
                        $query = "SELECT DebtorCode,PayDocDate,DocDate,companyname AS supp_name, PayDocNo AS receipt_no, PayDocDate AS receipt_date, GROUP_CONCAT(DISTINCT DocNo) AS t_inv_no, ROUND(SUM(InvApplyAmt),2) AS inv_apply_amount_total, ROUND(SUM(NetTotal),2) AS inv_amount_total,COUNT(*) as inv_count,ROUND(LocalPaymentAmt,2) as receipt_total,ROUND(LocalUnappliedAmount,2) as receipt_unapply,ROUND(KnockOffAmt,2) as knock_off_amount FROM b2b_invoice.`autocount_payment_hub` WHERE DebtorCode IN ($con_deb_code) GROUP BY PayDocNo";                    
                    }
                    else
                    {
                        // echo $user_guid;
                        ...
                        foreach ($posts->result() as $post)
                        {
                            ...
                            foreach($inv_no as $row)
                            {
                                ...
                                else
                                {
                                    // echo $row.'<br>';
                                    // echo strtok($row, '(');die;
                                    $inv_virtual_path = '<a href="https://b2b.xbridge.my/index.php/File_checking_autocount/Document_autocount?refno='.$row.'&doctype='.$inv_doc_type.'&supcode='.$post->DebtorCode.'&doctime='.$post->DocDate.' 00:00:00" target="_blank" > '.$row.'</a>'; 
                            ...
                            $nestedData['receipt_date'] = $post->receipt_date;
    ...
    public function official_invoice_table()
    {
            ...
            {     
                ...
                $columns = array(                  
                        ...
                        6   => 't_receipt_no',
                        7   => 'receipt_apply_total',
                        8   => 'receipt_count',
                        9   => 'cn_no',
                        10  => 'total_cn_amount',
                        11  => 'cn_apply_count',
                        12  => 'inv_payment_status_a_cn'
                );
                ...
                if($supplier_guid != '')
                {
                    ...
                    {                   
                        foreach ($posts->result() as $post)
                        {
                            ...
                            {
                                $row_no1 = $row;
                                $check_is_b2b_invoice = $this->db->query("SELECT * FROM b2b_invoice.autocount_payment_hub WHERE PayDocNo = '$row_no1'");
                                    // echo $row.'<br>';
                                    // echo strtok($row, '(');die;
                                $inv_virtual_path = '<a href="https://b2b.xbridge.my/index.php/File_checking_autocount/Document_autocount?refno='.$row_no1.'&doctype='.$doc_type.'&supcode='.$post->DebtorCode.'&doctime='.$check_is_b2b_invoice->row('PayDocDate').' 00:00:00" target="_blank" > '.$row.'</a>'; 
                                // echo $inv_virtual_path;die;
                            ...
                            {
                                $row_no2 = $row;
                                $check_is_b2b_invoice = $this->db->query("SELECT * FROM b2b_invoice.autocount_cn_hub WHERE CNDocNo = '$row_no2'");
                                    // echo $row.'<br>';
                                    // echo strtok($row, '(');die;
                                $inv_virtual_path = '<a href="https://b2b.xbridge.my/index.php/File_checking_autocount/Document_autocount?refno='.$row_no2.'&doctype='.$doc_type2.'&supcode='.$post->DebtorCode.'&doctime='.$check_is_b2b_invoice->row('CNDocDate').' 00:00:00" target="_blank" > '.$row.'</a>'; 
                                // echo $inv_virtual_path;die;
            ...
            {     
                ...
                if($supplier_guid != '')
                {
                    ...
                    { 
                        ...
                        $query = "SELECT DebtorCode,CompanyName AS supplier_name, CNDocNo, CNDocDate, ROUND(LocalTotal, 2) AS total_cn_amount, ROUND(RefundAmt, 2) AS cn_balance_amount, GROUP_CONCAT( DISTINCT DocNo) AS invoice_number, ROUND(KnockOffAmt, 2) AS apply_cn_amount, ROUND(SUM(NetTotal), 2) AS total_invoice_number, COUNT(DISTINCT DocNo) AS inv_count FROM b2b_invoice.`autocount_cn_hub` WHERE debtorcode IN ($con_deb_code) GROUP BY CNDocNo";                        
                    }
                    else
                    {
                        // echo $user_guid;
                        ...
                        foreach ($posts->result() as $post)
                        {
                            ...
                            foreach($inv_no as $row)
                            {
                                ...
                                else
                                {
                                    // echo $row.'<br>';
                                    // echo strtok($row, '(');die;
                                    $invoice_doc_date = $this->db->query("SELECT * FROM b2b_invoice.autocount_cn_hub WHERE docno = '".$row."'")->row('DocDate');
                                    // echo $this->db->last_query();die;
                            ...
                            $nestedData['CNDocDate'] = $post->CNDocDate;
                            $nestedData['total_cn_amount'] = $post->total_cn_amount;
