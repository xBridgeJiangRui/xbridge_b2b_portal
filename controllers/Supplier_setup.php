...
class Supplier_setup extends CI_Controller
{
    ...
    function index()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '')
        {
            ...
            $this->load->view('supplier_setup', $data);
            $this->load->view('supplier_setup_modal', $data);
  ...
  public function view_records()
           ...
           // echo $cross_check_reg_no ; echo " $$ " ; echo $cross_check_existed ;die;
            if($mode == 'create')
            {
                if($cross_check_reg_no == $cross_check_existed)
                {
                    ...
                    redirect('Supplier_setup?customer_guid='.$_SESSION['customer_guid']);
                };
            }
            else
            {
    ...
    public function delete_group()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '')
        {
            ...
            //echo $this->db->last_query();die;
            redirect('Supplier_setup?supplier_guid='.$supplier_guid); 
        }
        else
        {
            redirect('login_c');    
    ...
    public function change_cus_guid()
    {
        ...
        redirect('Supplier_setup?customer_guid='.$_REQUEST['customer_guid']);
    }
    ...
    public function get_supplier_multiple()
    {
        ...
        foreach($result->result() as $row)
        {
            ...
            else
            {
                ...
                $selected_supplier_code = $this->db->query("SELECT * FROM (SELECT * FROM set_supplier_user_relationship WHERE user_guid = '$user_guid') as a INNER JOIN (SELECT * FROM set_supplier_group) as b ON a.supplier_group_guid = b.supplier_group_guid INNER JOIN (SELECT * FROM set_supplier) as c ON a.supplier_guid = c.supplier_guid ");
            }
