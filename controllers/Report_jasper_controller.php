...
class Report_jasper_controller extends CI_Controller {
    ...
    public function setting()
    {
        ...
        { 
            $data = array(
                ...
                'report_setting' => $this->db->query("SELECT * FROM set_setting WHERE module_name = 'CONSIGNMENT'"),
                        );
            $this->load->view('header');
    ...
    {   
        ...
        {   
            ...
            $input_setting = $this->db->query("SELECT a.*,b.guid,IF(b.guid is null || b.guid = '',0,1) as sett FROM (SELECT * FROM jasper_report_setting WHERE isactive = 1 AND module_name = 'consignment_report') a LEFT JOIN (SELECT * FROM set_jasper_report_setting WHERE isactive = 1 AND report_module_ci_guid = '$request_link') b ON a.guid = b.jasper_report_setting_guid");

            $input_setting_array = array();
            foreach($input_setting->result() as $row)
            {
                if($row->sett == 1)
                {
                    $input_setting_array[] = $row->code;
                }
            }
            ...
            $data = array(
                'location' => $location,
                'code' => $code,
                'input_setting_array' => $input_setting_array
            );
            
            $this->load->view('header');
            $this->load->view('jasper_report_view_new',$data);
            $this->load->view('footer'); 
        }
        else
        {
    ...
    public function jasper_report_rest()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login())
        {  
