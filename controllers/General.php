...
class general extends CI_Controller
{
    ...
    public function view_status()
    {
        ...
        if($this->session->userdata('userid') == 'nabil.haziq@pandasoftware.my'){
            // $_SESSION['customer_guid'] = '8D5B38E931FA11E79E7E33210BD612D3'; //tf
            // $_SESSION['customer_guid'] = 'B00CA0BE403611EBA2FC000D3AC8DFD7'; //cks
            // $_SESSION['customer_guid'] = 'D361F8521E1211EAAD7CC8CBB8CC0C93'; //everrise
        }
    ...
    public function view_table()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            $acc_settings = $this->db->query("SELECT * FROM lite_b2b.acc_settings WHERE customer_guid = '".$_SESSION['customer_guid']."'");
            ...
            if (!empty($posts)) {
                ...
                foreach ($posts as $post) {
                    ...
                    if ($check_module == 'panda_prdncn') {
                        ...
                        if (in_array('VPOCITM', $_SESSION['module_code'])) 
                        {
                            $nestedData['refno'] = '<span style="display:flex;">' . $post->refno . '<i data-toggle="tooltip" data-placement="top" title="Click to preview item details" class="fa fa-info-circle" style="padding-top:5px;padding-left:10px;cursor: pointer;"  id="preview_doc_item_line" refno=' . $post->refno . ' doc_type=' . $post->type . '></i></span>';
                        } else {
                            $nestedData['refno'] = $post->refno;
                        }
    ...
    public function accept()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            $this->session->set_flashdata('message', 'PO Accept Action Session Expired! Please relogin');
            redirect('#');
        }
    }
    ...
    public function confirm()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            if ($from_module == 'panda_return_collection') // return collection accept collection detail
            {
                ...
                $check_status = $this->db->query("SELECT a.* FROM b2b_summary.dbnote_batch a 
                INNER JOIN lite_b2b.set_supplier_group b
                ON a.sup_code = b.supplier_group_name
                ...
                } else if($check_status->num_rows() > 0) {
                ...
                {
                    $this->session->set_flashdata('warning', 'Invalid Process');
    ...
    public function reject()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            if ($from_module == 'panda_po_2') {
                ...
                $refno_supplier_code = $refno_loc_group->row('SCode');
                ...
                WHERE a.supplier_group_name = '$refno_supplier_code'
                GROUP BY a.supplier_group_name")->row('subscribe_edi');

                if($subscribe_edi == '1')
                {
                    $check_postatus = $this->db->query("SELECT * from b2b_summary.pomain where refno = '$refno' and customer_guid = '$customer_guid' and status in ('', 'viewed', 'printed', 'Accepted')");
                    ...
                    if ($check_postatus->num_rows() != '1') {
                        ...
                        // redirect($_SESSION['frommodule'] . "?loc=" . $_REQUEST['loc']);
                        echo '<script>alert("Refno ' . $refno . ' fail to reject! Please check the status of the PO.");window.location.href = "'.site_url($_SESSION['frommodule'] . "?loc=" . $_REQUEST['loc']).'";</script>;';
                        die;
                    };
                }
                else
            ...
            if ($from_module == 'panda_return_collection') {
                ...
                if ($check_status->num_rows() != '1') {
                    echo '<script>alert("Refno ' . $refno . ' fail to reject! Please check the status of the Doc.");window.location.href = "'.site_url($_SESSION['frommodule'] . "?loc=" . $_REQUEST['loc']).'";</script>;';
                    die;
