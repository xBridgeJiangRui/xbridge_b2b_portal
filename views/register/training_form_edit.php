        ...
        <div class="box-header with-border">
          ...
          <br>
              ...
              <h4 class="text-bold part1" style="margin-left: 15px; ">Part 1: Organizational Information</h4><br>
                ...
                <form action="<?php echo site_url('Training_user/training_update')?>?training_guid=<?php echo $_REQUEST['training_guid'] ?>" method="post" id="myForm">
                    <div class="form-row">
                  ...
                  <?php if($training->num_rows() != 0)
                  {
                   ...
                   ?>
                    ...
                    {
                     ?>
                      <button type="button" style="float: right;" class="btn btn-xs btn-default"  <?php echo $disabled?> id="add_code_modal" training_guid ='<?php echo $_REQUEST['training_guid'] ?>' customer_guid ='<?php echo $customer_guid ?>'>Add Code</button>
                     <?php
                    }
                    ?>
                ...
                ?>
                  ...
                  <div class="form-group col-md-12">
                    ...
                    {
                     ?>
                      <button id="part_btn" type="button" class="btn btn-xs btn-default"  <?php echo $disabled?> style="float: right;margin-bottom:15px;" training_guid ='<?php echo $_REQUEST['training_guid'] ?>' customer_guid ='<?php echo $customer_guid ?>'><i class="glyphicon glyphicon-plus" aria-hidden="true" ></i> Add</button>
                     <?php
                    }
                    ?>
                    </h4>
                 ...
                 <h5 class="text-md-left ">
                  ...
                  <?php if(in_array('IAVA',$this->session->userdata('module_code')))
                  {
                   ?>
                    ...
                    <!-- <button title="Save" onclick="save_form()" data-toggle="modal" data-target="#saveModal" id="save_btn" type="button" class="btn btn-md btn-default" <?php echo $disabled?> ><i class="fa fa-save" aria-hidden="true"></i>&nbsp&nbspSave</button> -->
              ...
              <!-- Save alert -->
                ...
                <?php 
                  if($training->num_rows() != 0 )
                  {
                    foreach($training->result() as $key)
                    {
                      ...
                      if($form_status == 'New' || $form_status == 'Processing' || $form_status == 'Emailed' )
                      {
                        $disabled_special = ''; 
                      }
                      else
                ...
                <?php if(in_array('IAVA',$this->session->userdata('module_code')))
                  {
                   ?>
                    <button id="completebtn" type="button" class="btn btn-md btn-warning" training_guid ='<?php echo $_REQUEST['training_guid'] ?>' customer_guid ='<?php echo $customer_guid ?>'  <?php echo $disabled_special ?>><i class="fa fa-check" aria-hidden="true"></i>&nbspRegistered</button>
                   <?php
                  }
                  ?>
            </form> 
...
<script type="text/javascript">
$('document').ready(function(){
training_guid = "<?php echo $_REQUEST['training_guid'];?>";
...
$(document).on('click','#create_part',function(){
    ...
    $.ajax({
          url:"<?php echo site_url('Training_user/add_part_info');?>",
          method:"POST",
...
$(document).on('click','#edit_part',function(){
    ...
    $.ajax({
          url:"<?php echo site_url('Training_user/edit_part_info');?>",
          method:"POST",
...
$(document).on('click','#active_btn',function(){
  ...
  $.ajax({
    url:"<?php echo site_url('Training_user/active_status');?>",
    method:"POST",
...
$(document).on('click','#completebtn',function(){
  ...
  confirmation_modal('Registered Training Participant(s) : <b>' +no_part+'</b><br>Are you sure want <b> Registered </b>?');
  $(document).off('click', '#confirmation_yes').on('click', '#confirmation_yes', function(){
  $.ajax({
          url:"<?php echo site_url('Training_user/complete_status');?>",
          method:"POST",
          data:{customer_guid:customer_guid,training_guid:training_guid},
          beforeSend:function(){
            $('.btn').button('loading');
          },
          success:function(data)
          {
            ...
            if (json.para1 == '1') {
              ...
              setTimeout(function() {
              $('.btn').button('reset');
              window.location = "<?= site_url('Training_user/training_form_edit?training_guid=');?>"+training_guid;
              }, 300);
            }//close else
          }//close success
        });//close ajax
