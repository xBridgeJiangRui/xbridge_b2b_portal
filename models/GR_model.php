...
class GR_model extends CI_Model{
     ...
     function get_gr_list($sqlStatement,$limit, $start)
     {
    ...
    function countGR()
    {
        ...
        $result = $this->db->from('grmain')
                           ->join('customer_profile', 'customer_profile.customer_guid = grmain.customer_guid')
                           ->join('customer_supcus', 'customer_supcus.customer_guid = customer_profile.customer_guid and grmain.code = customer_supcus.code')
                           ->join('user_customer', 'user_customer.supcus_guid = customer_supcus.supcus_guid')
                           ->where('grmain.loc_group', $branch_code)
     ...
     function get_gr_details($sqlStatement)
     {
        $sql = $sqlStatement;
        $query = $this->db->query($sql);
        return $query->result();
